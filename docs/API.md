...
Call Builder users generate schedules through `POST /api/call-builder/generate` with `{ "blockNumber": 3 }`. Optional `lockedAssignments` are treated as hard choices and `baselineAssignments` are used only as the final minimum-change tie-breaker. The local CP-SAT solver returns an independently validated schedule plus `solverSummary`, including whether optimality was proven and the result of each hierarchical goal. Contradictory locks return `422`. `POST /api/call-builder/suggest` accepts the current assignments and returns a coordinated minimum-change improvement when one exists.
