...
The admin Users tab can add/delete users one at a time or in bulk, generate temporary passwords, copy privileges from another user, and grant per-service privileges. A student account is case-assignable, and its egg icon opens [Surgemon](https://surgemon.com/) rather than the Tamagotchi. Residents and attendings may also receive the Advanced editor setting (`canBuildCall`), which unlocks both Call Builder and Blocks.
...
User listing and non-admin privilege changes accept either a logged-in admin browser-session bearer token or `ADMIN_API_KEY`. API-key updates may change fields such as `servicePrivileges`, `canAddContacts`, and `canBuildCall`, but cannot modify an admin browser account, change account roles, or relink attending identities. Account deletion still requires a logged-in admin browser session:
...
Grant Call Builder access to an existing non-admin account:

```bash
curl -X PATCH https://your-domain.example/api/users/jsmith \
...
Call Builder users generate schedules through `POST /api/call-builder/generate` with `{ "blockNumber": 3 }`. Optional `lockedAssignments` are treated as hard choices and `baselineAssignments` are used only as the final minimum-change tie-breaker. A PGY-4/5 resident may fill either a `senior` or `mid-level` assignment; other PGY levels retain their normal position. The hierarchy first minimizes burdens above two call equivalents, then minimizes PGY-4/5 mid-level cross-coverage, so chiefs cover mid-level call only when needed to avoid a worse load result. The local CP-SAT solver returns an independently validated schedule plus `solverSummary`, including whether optimality was proven and the result of each hierarchical goal. Contradictory locks return `422`. `POST /api/call-builder/suggest` accepts the current assignments and returns a coordinated minimum-change improvement when one exists.
...
When both dates are supplied to `GET /api/attending-coverage`, the response includes `effectiveCoverage` in addition to the canonical stored rows. It identifies night-to-day and weekend-day inheritance, expands shorthand weekend rows, and includes the Monday `earlyMorningUntil6am` handoff period, so API readers do not need to reproduce those rules.
