...
- Accounts have one of four types: resident, attending, student, or admin. Admins get a Users tab for single or bulk account creation, deletion, temporary-password resets, attending identity links, and per-service `view`, `request`, or `edit` privileges. New non-admin accounts default to view-only access.
...
- A dedicated per-user **Advanced editor** setting is available to residents and attendings. It unlocks the block-aware Call Builder plus the Blocks tab for changing resident rotation blocks, vacation ranges, and other dated time off with a free-text scheduling note. These edits update the shared database directly. Admins can grant or revoke the setting from the Users tab.
...
- AI agents can read the public landing page at `/agent` or fetch its JSON quick guide at `/api` or `/api/agent-guide` before authenticating.
