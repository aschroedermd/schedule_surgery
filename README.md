...
- A dedicated per-user **Call Builder** privilege unlocks a block-aware resident call scheduler. It combines rotations, PGY levels, vacation and approved-unavailable dates, resident requests, and prior published call into a deterministic first draft; scores fairness and rule adherence; supports manual assignment edits; suggests improvements; and blocks publishing until all hard validity errors are resolved. Admins can grant the privilege from the Users tab.
