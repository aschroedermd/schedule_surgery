...
- The signed-in landing page is a schedule assistant powered by OpenRouter. It defaults to `deepseek/deepseek-v4-flash`, with `google/gemma-3-27b-it` as its automatic fallback; an admin session or `ADMIN_API_KEY` can read or change the persisted model ids through `/api/admin/chat-settings`. It can read OR/clinic, call, calendar, and vacation context through permission-aware tools. Each user has 20 requests per Eastern-time day and receives a warning at five remaining.
